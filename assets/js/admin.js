...
(function () {
  ...
  function enter() {
    ...
    $("#whoami").textContent = "داخل برقم " + S.session();
    renderAll();
  }
  ...
  function renderHome() {
    ...
    $("#kpis").innerHTML =
      ...
      kpi("box",   ar(S.data.packages.length), "باقة معروضة") +
    ...
    $("#lastBooks").innerHTML = last.length ? `<table><thead><tr>
        <th>العميل</th><th>المناسبة</th><th>التاريخ</th><th>الحالة</th></tr></thead><tbody>` +
      last.map((x) => `<tr>
  ...
  function itemModal(i) {
    const isNew = i == null;
    const it = isNew ? { id: S.uid("g"), name: "", cat: S.cats()[0] || "كوش", desc: "", price: 0, featured: false, slug: "", img: "" } : S.data.items[i];
    openModal(`
      ...
      <div class="field"><label>اسم المنتج</label><input id="fName" value="${esc(it.name)}" placeholder="مثال: كوشة الياسمين"></div>
      <div class="grid2">
  ...
  function renderPkgs() {
    ...
    $$("#pgrid [data-del]").forEach((b) => (b.onclick = () => {
      const i = +b.dataset.del;
      if (!confirm(`تحذف باقة «${ps[i].name}»؟`)) return;
  ...
  function pkgModal(i) {
    ...
    openModal(`
      <h3>${isNew ? "باقة جديدة" : "تعديل الباقة"}</h3>
      <p class="sub">سمّيها اسم يتفكر — ده اللي بيفضل في دماغ العميل.</p>
      <div class="grid2">
        <div class="field"><label>اسم الباقة</label><input id="pName" value="${esc(p.name)}" placeholder="مثال: ليلة قمرا"></div>
    ...
    $("#mSave").onclick = () => {
      ...
      if (!name) return toast("الباقة محتاجة اسم");
      Object.assign(p, {
      ...
      S.save(); closeModal(); toast(isNew ? "اتضافت الباقة" : "اتحفظت");
    };
  }
  ...
  function renderBooks() {
    ...
    $("#bookTbl").innerHTML = rows.length ? `<table><thead><tr>
      <th>العميل</th><th>المناسبة</th><th>الميعاد</th><th>الضيوف</th><th>الباقة</th><th>الحالة</th><th>وصل</th><th></th>
  ...
  $("#expCsv").onclick = () => {
    const H = ["الاسم", "الموبايل", "المناسبة", "التاريخ", "الضيوف", "الباقة", "الحالة", "ملاحظات", "وصل في"];
    const q = (v) => `"${String(v == null ? "" : v).replace(/"/g, '""')}"`;
    const csv = "﻿" + [H.map(q).join(",")].concat(
      S.data.bookings.map((b) => [b.name, b.phone, b.type, b.date, b.guests, b.pkg, b.status, b.note, new Date(b.created).toLocaleString("ar-EG")].map(q).join(","))
    ).join("\n");
    dl(new Blob([csv], { type: "text/csv;charset=utf-8" }), "richy-bookings.csv");
    toast("اتنزل الملف");
  };
  ...
  $("#expJson").onclick = () => { dl(S.export(), "richy-backup.json"); toast("اتنزلت النسخة"); };
  ...
  document.addEventListener("richy:change", () => { renderHome(); renderItems(); renderPkgs(); renderBooks(); });
})();
