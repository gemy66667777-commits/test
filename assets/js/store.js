...
(function (w) {
  ...
  let db = read();
  if (!db) db = write(clone(w.RICHY_SEED));

  /* دمج أي حقول جديدة اتضافت للبيانات الأساسية بعد آخر حفظ */
  for (const k in w.RICHY_SEED.settings)
    if (!(k in db.settings)) db.settings[k] = w.RICHY_SEED.settings[k];
  for (const k of ["services", "testimonials"])
    if (!db[k] || !db[k].length) db[k] = clone(w.RICHY_SEED[k]);
  ...
  const Store = {
    ...
    save() { write(db); document.dispatchEvent(new CustomEvent("richy:change")); return db; },

    reset() { db = write(clone(w.RICHY_SEED)); document.dispatchEvent(new CustomEvent("richy:change")); return db; },
    ...
    import(json) {
      ...
      db = write(d);
      document.dispatchEvent(new CustomEvent("richy:change"));
    },
    ...
    login(user, pass) {
      ...
      if (ok) sessionStorage.setItem("richy.session", u);
      return ok;
    },
    session() { return sessionStorage.getItem("richy.session"); },
    logout() { sessionStorage.removeItem("richy.session"); }
  };
