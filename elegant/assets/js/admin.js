...
(function () {
  ...
  $("#sBrand").textContent = S.s.latin || S.s.brand;
  ...
  function itemModal(i) {
    const isNew = i == null;
    const it = isNew ? { id: S.uid("g"), name: "", cat: S.cats()[0] || "خطوبة", desc: "", price: 0, featured: false, slug: "", img: "" } : S.data.items[i];
    openModal(`
      ...
      <div class="field"><label>اسم المنتج</label><input id="fName" value="${esc(it.name)}" placeholder="مثال: كوشة ورد أحمر وشمعدانات"></div>
      <div class="grid2">
  ...
  function pkgModal(i) {
    ...
    openModal(`
      ...
      <div class="grid2">
        <div class="field"><label>اسم الباقة</label><input id="pName" value="${esc(p.name)}" placeholder="مثال: باقة السبوع"></div>
  ...
  $("#expCsv").onclick = () => {
    ...
    dl(new Blob([csv], { type: "text/csv;charset=utf-8" }), "elegant-bookings.csv");
    toast("اتنزل الملف");
  };
  ...
  $("#setForm").addEventListener("submit", (e) => {
    ...
    S.save();
    $("#sBrand").textContent = s.latin || s.brand;
    toast("اتحفظت الإعدادات ✦ الموقع اتحدّث");
  });

  $("#expJson").onclick = () => { dl(S.export(), "elegant-backup.json"); toast("اتنزلت النسخة"); };
  ...
  document.addEventListener("elegant:change", () => { renderHome(); renderItems(); renderPkgs(); renderBooks(); });
})();
