...
(function () {
  ...
  const ICON = {
    arch:     sv('<path d="M4.5 21V11a7.5 7.5 0 0 1 15 0v10"/><path d="M8.5 21v-10a3.5 3.5 0 0 1 7 0v10"/>'),
    board:    sv('<rect x="3.6" y="3.6" width="16.8" height="13.4" rx="1.8"/><path d="M12 17v4M8.6 21h6.8"/><path d="M7.4 8.2h9.2M7.4 12h5.6"/>'),
    ...
    neon:     sv('<path d="M3.5 15.5c2-4.5 4-6.7 6-6.7s2 4.4 4 4.4 3.5-2.2 5.5-6.2"/><path d="M6 20.5h12"/><path d="M9.5 20.5v-2M14.5 20.5v-2"/>'),
    chair:    sv('<path d="M6.5 11V5.6a2 2 0 0 1 2-2h7a2 2 0 0 1 2 2V11"/><path d="M4.8 11h14.4v3.2a2 2 0 0 1-2 2H6.8a2 2 0 0 1-2-2z"/><path d="M7 16.4V21M17 16.4V21"/>'),
    building: sv('<path d="M3.6 21V6.4a1.6 1.6 0 0 1 1.6-1.6h6.2a1.6 1.6 0 0 1 1.6 1.6V21"/><path d="M13 10.6h5.4A1.6 1.6 0 0 1 20 12.2V21"/><path d="M2.4 21h19.2"/><path d="M6.6 8.6h3.4M6.6 12.2h3.4M6.6 15.8h3.4M16 14h1.6M16 17.4h1.6"/>'),
    crown:    sv('<path d="M3.4 8.4 6.6 14l2.9-6.6L12 13l2.5-5.6L17.4 14l3.2-5.6L19 19.4H5z"/><path d="M5 19.4h14"/>'),
  ...
  function paintSettings() {
    ...
    document.title = `${s.latin} | ${s.brand} — ${s.tagline}`;

    $("#heroTags").innerHTML = S.data.services.map((x) => `<span>${esc(x.name)}</span>`).join("");

    const occ = S.data.occasions || [];
    $("#occStrip").innerHTML = occ.map((o) => `<span>${esc(o)}</span>`).join("");
    const ts = $("#bType");
    if (ts) ts.innerHTML = occ.map((o) => `<option>${esc(o)}</option>`).join("") + `<option>مناسبة تانية</option>`;
    ...
    wa.href = `https://wa.me/${String(s.whatsapp).replace(/\D/g, "")}?text=` +
      encodeURIComponent(`السلام عليكم، حابة أستفسر عن تنظيم مناسبة مع ${s.brand} 🌿`);
    ...
    $("#conGrid").innerHTML = `
      ...
      <div class="con"><span class="ic">${ICON.pin}</span><h3>بننفّذ فين</h3>
        <p>${esc(s.address)}</p>
        <a href="https://www.google.com/maps/search/${encodeURIComponent(s.address)}" target="_blank" rel="noopener">افتح على الخريطة</a></div>
      <div class="con"><span class="ic">${ICON.clock}</span><h3>مواعيد الرد</h3><p>${esc(s.hours)}</p>
  ...
  function paintPackages() {
    $("#pkgGrid").innerHTML = S.data.packages.map((p) => `
      <article class="pkg rv${p.badge ? " hot" : ""}">
        ...
        <button class="btn ${p.badge ? "" : "ghost"}" data-pick="${esc(p.name)}">اطلبي الباقة</button>
      </article>`).join("");

    const sel = $("#bPkg");
    sel.innerHTML = `<option value="">مش محددة / حابة أستشير</option>` +
  ...
  /* ---------- آراء ---------- */
  function paintSays() {
    const box = $("#sayGrid"); if (!box) return;
    box.innerHTML = S.data.testimonials.map((t) => `
  ...
  function welcomeBoard() {
    ...
    const A = $("#wbA"), B = $("#wbB"), D = $("#wbD");
    const na = $("#wbNa"), nb = $("#wbNb"), dt = $("#wbDt");

    function draw() {
      na.textContent = A.value.trim() || "اسم العروسة";
      nb.textContent = B.value.trim() || "اسم العريس";
      if (D.value) {
        const p = D.value.split("-");
        dt.textContent = (+p[2]) + " . " + (+p[1]) + " . " + p[0];
      } else dt.textContent = "— . — . ————";
    }
    [A, B, D].forEach((el) => el.addEventListener("input", draw));

    $$(".modes button").forEach((btn) => btn.addEventListener("click", () => {
      $$(".modes button").forEach((x) => x.classList.toggle("on", x === btn));
      const neon = btn.dataset.mode === "neon";
      wb.classList.toggle("neon-mode", neon);
      wb.classList.toggle("board-mode", !neon);
    }));

    $("#wbOrder").addEventListener("click", () => {
      const mode = $(".modes button.on").dataset.mode === "neon" ? "نيون" : "لوح ترحيب";
      $("#bNote").value = "عايزة " + mode + " بالأسامي: " + na.textContent + " & " + nb.textContent +
                          (D.value ? " — التاريخ " + dt.textContent : "");
      if (D.value) $("#bDate").value = D.value;
      $("#book").scrollIntoView({ behavior: "smooth" });
      setTimeout(() => $("#bName").focus({ preventScroll: true }), 600);
    });
    draw();
  ...
  $("#bookForm").addEventListener("submit", (e) => {
    ...
    if (name.length < 3 || phone.replace(/\D/g, "").length < 10) {
      ok.textContent = "اكتبي اسمك ورقم موبايل صحيح عشان نقدر نرجعلك.";
  ...
  document.addEventListener("elegant:change", render);
  addEventListener("storage", (e) => { if (e.key === "elegant.v1") location.reload(); });
  welcomeBoard();
