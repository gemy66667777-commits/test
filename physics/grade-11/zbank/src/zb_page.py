...
def choices(ch, cols=None, narrow=False):
    ...
    return ('<div class="ch%s">' % cls +
            ''.join('<span class="qtext"><b>%s)</b>%s</span>' % (l, c) for l, c in zip('abcd', ch)) +
            '</div>')
...
def part(k, p, narrow=False):
    ...
    return ('<div class="part"><span class="rn">%s</span><div class="qtext">%s</div>%s</div>'
            '<div class="space %s"></div>' % (ROMAN[k], p['t'], choices(p['ch'], p.get('cols'), narrow)
                                              if p.get('ch') else '', sp))


def question(n, stem, parts, fig=None, figpos='below'):
    head = ('<div class="stem">' + FR.bubble(n) + '<div class="qtext">' + stem + '</div>' +
            FR.stem_doodle() + '</div>')
