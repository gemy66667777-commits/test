...
(function () {
  ...
  function wire(root, key, form, label) {
    ...
    $$(root + " [data-mv]").forEach((b) => (b.onclick = () => {
      ...
      db = MStore.patch((d) => { const t = d[key][i]; d[key][i] = d[key][j]; d[key][j] = t; });
    }));
    $$(root + " [data-del]").forEach((b) => (b.onclick = () => {
      const i = +b.dataset.del;
      if (!confirm("تحذف " + label(db[key][i]) + "؟")) return;
      db = MStore.patch((d) => d[key].splice(i, 1)); toast("اتحذف");
    }));
  }
    ...
    $("#ogrid").innerHTML = os.length ? os.map((o, i) => {
      ...
      return `<article class="mcard">
        ...
        <div class="bd"><b>${esc(o.name)}</b>
          <small>${ar(c.pieces)} خدمة · ${ar(c.n)} صنف · قبل ${ar(c.was)}</small>
  ...
  function offerForm(i) {
    ...
    openModal(`
      ...
      <div class="field"><label>عدد الخدمات المكتوب</label><input id="oPieces" type="number" min="0" value="${Number(o.pieces) || 0}">
    ...
    function recalc() {
      ...
      let html = `<span>المحسوب:</span> <b>${ar(c.pcs)} خدمة</b> <span>·</span> <b>${ar(c.sum)} ${esc(cur())}</b>`;
      ...
      if (manualPcs && manualPcs !== c.pcs)
        html += ` <span class="warn">⚠ عدد الخدمات المكتوب ${ar(manualPcs)} مختلف عن المحسوب ${ar(c.pcs)}</span>`;
  ...
  function renderCats() {
    ...
    $$("#catsTbl [data-mv]").forEach((b) => (b.onclick = () => {
      ...
      db = MStore.patch((d) => { const t = d.cats[i]; d.cats[i] = d.cats[j]; d.cats[j] = t; });
    }));
    $$("#catsTbl [data-del]").forEach((b) => (b.onclick = () => {
      ...
      db = MStore.patch((d) => d.cats.splice(i, 1)); toast("اتحذف");
    }));
  }
  ...
  function catForm(i) {
    ...
    $("#mS").onclick = () => {
      ...
      db = MStore.patch((d) => { i < 0 ? d.cats.push({ id: c.id, name: n }) : (d.cats[i].name = n); });
      closeModal(); toast("اتحفظ ✅");
    };
  }
  ...
  function picForm(key, i, title, ph) {
    ...
    $("#mS").onclick = () => {
      ...
      db = MStore.patch((d) => { i < 0 ? d[key].push(rec) : (d[key][i] = rec); });
      closeModal(); toast("اتحفظ ✅");
    };
  }
  ...
  function brForm(i) {
    ...
    $("#mS").onclick = () => {
      ...
      db = MStore.patch((d) => { i < 0 ? d.branches.push(rec) : (d.branches[i] = rec); });
      closeModal(); toast("اتحفظ ✅");
    };
  }
  ...
  $("#expCsv").onclick = () => {
    ...
    dl(new Blob([csv], { type: "text/csv;charset=utf-8" }), "mrmedia-orders.csv");
  };
  ...
  const KEYS = ["brandAr","brand","tagline","slogan","intro","ship","phone","whats","whats2",
                "hours","priceNote","priceHint","photoNote","currency","adminPass","insta"];
  const F = $("#setForm");
  function renderSet() {
    const s = db.settings;
  ...
  $("#expJson").onclick = () =>
    dl(new Blob([JSON.stringify(db, null, 2)], { type: "application/json" }), "mrmedia-backup.json");
  ...
  window.addEventListener("mrmedia:change:full", () =>
