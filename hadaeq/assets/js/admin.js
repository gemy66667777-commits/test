...
(function () {
  ...
  function wire(root, key, form, label) {
    ...
    $$(root + " [data-mv]").forEach((b) => (b.onclick = () => {
      ...
      db = HStore.patch((d) => { const t = d[key][i]; d[key][i] = d[key][j]; d[key][j] = t; });
    }));
    $$(root + " [data-del]").forEach((b) => (b.onclick = () => {
      const i = +b.dataset.del;
      if (!confirm("تحذف " + label(db[key][i]) + "؟")) return;
      db = HStore.patch((d) => d[key].splice(i, 1)); toast("اتحذف");
    }));
  }
  ...
  function prodForm(i) {
    ...
    $("#mS").onclick = () => {
      ...
      db = HStore.patch((d) => { i < 0 ? d.products.push(rec) : (d.products[i] = rec); });
  ...
  function renderCats() {
    ...
    $$("#catsTbl [data-mv]").forEach((b) => (b.onclick = () => {
      ...
      db = HStore.patch((d) => { const t = d.cats[i]; d.cats[i] = d.cats[j]; d.cats[j] = t; });
    }));
    $$("#catsTbl [data-del]").forEach((b) => (b.onclick = () => {
      ...
      db = HStore.patch((d) => d.cats.splice(i, 1)); toast("اتحذف");
    }));
  }
  ...
  function catForm(i) {
    ...
    $("#mS").onclick = () => {
      ...
      db = HStore.patch((d) => { i < 0 ? d.cats.push({ id: c.id, name: n }) : (d.cats[i].name = n); });
      closeModal(); toast("اتحفظ ✅");
    };
  }
  ...
  function picForm(key, i, title, ph) {
    ...
    $("#mS").onclick = () => {
      ...
      db = HStore.patch((d) => { i < 0 ? d[key].push(rec) : (d[key][i] = rec); });
      closeModal(); toast("اتحفظ ✅");
    };
  }
  ...
  function brForm(i) {
    ...
    $("#mS").onclick = () => {
      ...
      db = HStore.patch((d) => { i < 0 ? d.branches.push(rec) : (d.branches[i] = rec); });
      closeModal(); toast("اتحفظ ✅");
    };
  }
  ...
  function renderOrds() {
    ...
    $$("#ordsTbl [data-st]").forEach((s) => (s.onchange = () => {
      db = HStore.patch((d) => (d.orders[+s.dataset.st].status = s.value)); toast("الحالة اتغيّرت");
    }));
    $$("#ordsTbl [data-del]").forEach((b) => (b.onclick = () => {
      db = HStore.patch((d) => d.orders.splice(+b.dataset.del, 1)); toast("اتحذف");
    }));
  }
  ...
  $("#expCsv").onclick = () => {
    ...
    dl(new Blob([csv], { type: "text/csv;charset=utf-8" }), "hadaeq-orders.csv");
  };
  ...
  const KEYS = ["brandAr","brand","tagline","slogan","intro","ship","phone","whats","whats2",
                "hours","priceNote","priceHint","photoNote","currency","adminPass","snap"];
  const F = $("#setForm");
  function renderSet() {
    const s = db.settings;
  ...
  $("#expJson").onclick = () =>
    dl(new Blob([JSON.stringify(db, null, 2)], { type: "application/json" }), "hadaeq-backup.json");
  ...
  $("#impFile").onchange = (e) => {
    ...
    rd.onload = () => {
      try {
        ...
        db = HStore.set(o); toast("النسخة اترجّعت ✅");
  ...
  $("#resetAll").onclick = () => {
    ...
    HStore.reset(); db = HStore.get(); toast("رجع للأصل");
  };
  ...
  window.addEventListener("hadaeq:change:full", () =>
    toast("مساحة التخزين اتملت — امسح صور قديمة أو صغّر الصور"));
  HStore.on(() => { db = HStore.get(); if ($("#app").classList.contains("on")) renderAll(); });
})();
