...
(function () {
  ...
  function renderHome() {
    ...
    $("#lastOrds").innerHTML = last.length ? `<table><thead><tr>
        <th>العميلة</th><th>المطلوب</th><th>المنطقة</th><th>التاريخ</th></tr></thead><tbody>` +
      last.map((x) => `<tr>
  ...
  function renderProds() {
    ...
    $("#pgrid").innerHTML = ps.length ? ps.map((p, i) => `
      <article class="mcard">
        ...
        <div class="bd"><b>${esc(p.name)}</b><small>${esc(deptName(p.cat))}</small>
    ...
    $$("#pgrid [data-mv]").forEach((b) => (b.onclick = () => {
      ...
      db = MStore.patch((d) => { const t = d.products[i]; d.products[i] = d.products[j]; d.products[j] = t; });
    }));
    $$("#pgrid [data-del]").forEach((b) => (b.onclick = () => {
      const i = +b.dataset.del;
      if (!confirm("تحذف «" + db.products[i].name + "»؟")) return;
      db = MStore.patch((d) => d.products.splice(i, 1)); toast("اتحذف");
    }));
  }
  $("#addProd").onclick = () => prodForm(-1);

  const deptName = (id) => (db.cats.filter((d) => d.id === id)[0] || {}).name || "—";

  function prodForm(i) {
    const p = i < 0
      ? { id: "p" + Date.now(), name: "", img: "assets/img/p-pots.jpg", price: 0, cat: db.cats[0].id, note: "" }
    ...
    openModal(`
      ...
      <div class="field"><label>اسم المنتج</label><input id="mName" value="${esc(p.name)}" placeholder="مثال: طقم بورسلين مخطط كحلي"></div>
    ...
    $("#mS").onclick = () => {
      ...
      const rec = { id: p.id, name: n, img: img, price: Number($("#mPrice").value) || 0,
        cat: $("#mDept").value, note: $("#mNote").value.trim() };
      db = MStore.patch((d) => { i < 0 ? d.products.push(rec) : (d.products[i] = rec); });
  ...
  function renderOffers() {
    ...
    $("#ogrid").innerHTML = os.length ? os.map((o, i) => {
      ...
      return `<article class="mcard">
        <div class="ph">${o.tag ? `<span class="fav">${esc(o.tag)}</span>` : ""}
          <img src="${esc(its[0] ? its[0].img : "assets/img/p-pots.jpg")}" alt="" loading="lazy"></div>
    ...
    $$("#ogrid [data-mv]").forEach((b) => (b.onclick = () => {
      ...
      db = MStore.patch((d) => { const t = d.offers[i]; d.offers[i] = d.offers[j]; d.offers[j] = t; });
    }));
    $$("#ogrid [data-del]").forEach((b) => (b.onclick = () => {
      const i = +b.dataset.del;
      if (!confirm("تحذف «" + db.offers[i].name + "»؟")) return;
      db = MStore.patch((d) => d.offers.splice(i, 1)); toast("اتحذف");
    }));
  }
  ...
  function offerForm(i) {
    ...
    $("#mS").onclick = () => {
      ...
      db = MStore.patch((d) => { i < 0 ? d.offers.push(rec) : (d.offers[i] = rec); });
      closeModal(); toast("اتحفظ ✅");
    };
  }
  ...
  function renderItems() {
    ...
    $$("#mgrid [data-mv]").forEach((b) => (b.onclick = () => {
      ...
      db = MStore.patch((d) => { const t = d.gallery[i]; d.gallery[i] = d.gallery[j]; d.gallery[j] = t; });
    }));
    $$("#mgrid [data-del]").forEach((b) => (b.onclick = () => {
      const i = +b.dataset.del;
      if (!confirm("تحذف الصورة دي؟")) return;
      db = MStore.patch((d) => d.gallery.splice(i, 1)); toast("اتحذفت");
    }));
  }
  $("#addItem").onclick = () => itemForm(-1);
  function itemForm(i) {
    const it = i < 0 ? { img: "assets/img/p-pots.jpg", cap: "" } : Object.assign({}, db.gallery[i]);
    openModal(`
      ...
      <div class="field"><label>الصورة</label>
        <div class="drop" id="mPick"><img id="mImg" src="${esc(it.img)}" alt="">
          <div>اضغط لاختيار صورة من جهازك</div></div></div>
      <div class="field"><label>الوصف اللي تحت الصورة</label><input id="mCap" value="${esc(it.cap)}" placeholder="مثال: طقم حلل جرانيت بمقابض خشب"></div>
      <div class="modal-acts"><button class="btn ghost" id="mC">إلغاء</button><button class="btn" id="mS">حفظ</button></div>`);
    let img = it.img;
    $("#mPick").onclick = () => pickImg((d) => { img = d; $("#mImg").src = d; });
    $("#mC").onclick = closeModal;
    $("#mS").onclick = () => {
      ...
      db = MStore.patch((d) => { i < 0 ? d.gallery.push(rec) : (d.gallery[i] = rec); });
      closeModal(); toast("اتحفظ ✅");
    };
  }
  ...
  function renderOrds() {
    ...
    $("#ordsTbl").innerHTML = rows.length ? `<table><thead><tr>
        <th>التاريخ</th><th>العميلة</th><th>المطلوب</th><th>المنطقة</th><th>النوع</th><th>الحالة</th><th></th></tr></thead><tbody>` +
      rows.map(({ b, i }) => `<tr>
        <td>${esc(fmt(b.at))}</td>
    ...
    $$("#ordsTbl [data-st]").forEach((s) => (s.onchange = () => {
      db = MStore.patch((d) => (d.orders[+s.dataset.st].status = s.value)); toast("الحالة اتغيّرت");
    }));
    $$("#ordsTbl [data-del]").forEach((b) => (b.onclick = () => {
      db = MStore.patch((d) => d.orders.splice(+b.dataset.del, 1)); toast("اتحذف");
    }));
  }
  ...
  $("#expCsv").onclick = () => {
    ...
    dl(new Blob([csv], { type: "text/csv;charset=utf-8" }), "mazaya-orders.csv");
  };
  ...
  function renderSet() {
    const s = db.settings;
    ["brandAr","brand","tagline","intro","addr","mapq","ship","phone","whats","hours","priceNote","priceHint","currency","adminPass"].forEach((k) => {
  ...
  $("#expJson").onclick = () =>
    dl(new Blob([JSON.stringify(db, null, 2)], { type: "application/json" }), "mazaya-backup.json");
  ...
  $("#impFile").onchange = (e) => {
    ...
    rd.onload = () => {
      try {
        ...
        db = MStore.set(o); toast("النسخة اترجّعت ✅");
  ...
  $("#resetAll").onclick = () => {
    ...
    MStore.reset(); db = MStore.get(); toast("رجع للأصل");
  };

  /* ---------- تشغيل ---------- */
  function renderAll() { renderHome(); renderProds(); renderOffers(); renderItems(); renderOrds(); renderSet(); }
  MStore.on(() => { db = MStore.get(); if ($("#app").classList.contains("on")) renderAll(); });
})();
