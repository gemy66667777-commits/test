...
(function (w) {
  ...
  let db = read();
  if (!db) db = write(clone(w.GENSITA_SEED));

  /* دمج أي حقول جديدة اتضافت للبيانات الأساسية بعد آخر حفظ */
  for (const k in w.GENSITA_SEED.settings)
    if (!(k in db.settings)) db.settings[k] = w.GENSITA_SEED.settings[k];
  for (const k of ["services", "testimonials"])
    if (!db[k] || !db[k].length) db[k] = clone(w.GENSITA_SEED[k]);
  ...
  const Store = {
    ...
    save() { write(db); document.dispatchEvent(new CustomEvent("gensita:change")); return db; },

    reset() { db = write(clone(w.GENSITA_SEED)); document.dispatchEvent(new CustomEvent("gensita:change")); return db; },
    ...
    import(json) {
      ...
      db = write(d);
      document.dispatchEvent(new CustomEvent("gensita:change"));
    },
    ...
    login(user, pass) {
      ...
      if (ok) sessionStorage.setItem("gensita.session", u);
      return ok;
    },
    session() { return sessionStorage.getItem("gensita.session"); },
    logout() { sessionStorage.removeItem("gensita.session"); }
  };
