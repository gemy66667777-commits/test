...
(function () {
  ...
  function itemModal(i) {
    const isNew = i == null;
    const it = isNew ? { id: S.uid("g"), name: "", cat: S.cats()[0] || "كوش وأقواس", desc: "", price: 0, featured: false, slug: "", img: "" } : S.data.items[i];
    openModal(`
      ...
      <div class="field"><label>اسم المنتج</label><input id="fName" value="${esc(it.name)}" placeholder="مثال: قوس الغروب"></div>
      <div class="grid2">
  ...
  function pkgModal(i) {
    ...
    openModal(`
      ...
      <div class="grid2">
        <div class="field"><label>اسم الباقة</label><input id="pName" value="${esc(p.name)}" placeholder="مثال: فل وياسمين"></div>
  ...
  $("#expCsv").onclick = () => {
    ...
    dl(new Blob([csv], { type: "text/csv;charset=utf-8" }), "gensita-bookings.csv");
    toast("اتنزل الملف");
  };
  ...
  $("#expJson").onclick = () => { dl(S.export(), "gensita-backup.json"); toast("اتنزلت النسخة"); };
  ...
  document.addEventListener("gensita:change", () => { renderHome(); renderItems(); renderPkgs(); renderBooks(); });
})();
