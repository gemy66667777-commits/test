...
(function () {
  ...
  function paintSettings() {
    ...
    wa.href = `https://wa.me/${String(s.whatsapp).replace(/\D/g, "")}?text=` +
      encodeURIComponent(`السلام عليكم، حابة أستفسر عن تنسيق مناسبة مع ${s.brand} 🌿`);
    ...
    $("#conGrid").innerHTML = `
      ...
      <div class="con"><span class="ic">${ICON.pin}</span><h3>بنوصل فين</h3>
        <p>${esc(s.address)}</p>
  ...
  document.addEventListener("gensita:change", render);
  addEventListener("storage", (e) => { if (e.key === "gensita.v1") location.reload(); });
