...
(function () {
  ...
  const ICON = {
    ...
    light:  sv('<path d="M12 3v1.6M4.5 6.3l1.2 1.1M19.5 6.3l-1.2 1.1"/><path d="M12 5.6a5.2 5.2 0 0 0-2.9 9.5c.4.3.6.8.6 1.3v.8h4.6v-.8c0-.5.2-1 .6-1.3A5.2 5.2 0 0 0 12 5.6z"/><path d="M9.7 19.4h4.6M10.6 21.5h2.8"/>'),
    ...
    tray:   sv('<path d="M3 13.5h18"/><path d="M4.5 13.5a7.5 7.5 0 0 1 15 0"/><path d="M5 16.6h14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2z"/><path d="M12 6.4V4.2"/>'),
    board:  sv('<rect x="3.2" y="4.5" width="17.6" height="12.4" rx="2.2"/><path d="M12 16.9V21M8.5 21h7"/><path d="M7.5 9.2h9M7.5 12.4h5.5"/>'),
    home:   sv('<path d="M3.5 10.6 12 3.5l8.5 7.1"/><path d="M5.5 12.3V20a.9.9 0 0 0 .9.9h11.2a.9.9 0 0 0 .9-.9v-7.7"/><path d="M10 20.9v-5h4v5"/>'),
    heart:  sv('<path d="M12 20.4S3.8 15.3 3.8 9.6a4.4 4.4 0 0 1 8.2-2.3 4.4 4.4 0 0 1 8.2 2.3c0 5.7-8.2 10.8-8.2 10.8z" fill="currentColor" stroke="none"/>'),
  ...
  function paintSettings() {
    ...
    document.title = `${s.brand} | ${s.latin} — ${s.tagline}`;
    ...
    wa.href = `https://wa.me/${String(s.whatsapp).replace(/\D/g, "")}?text=` +
      encodeURIComponent(`السلام عليكم، حابة أستفسر عن تجهيز مناسبة مع ${s.brand} 🌿`);
    ...
    $("#conGrid").innerHTML = `
      ...
      <div class="con"><span class="ic">${ICON.pin}</span><h3>بنجهّز فين</h3>
        <p>${esc(s.address)}</p>
  ...
  function paintAddons() {
    const box = $("#addGrid"); if (!box) return;
    box.innerHTML = (S.data.addons || []).map((a) => `
      <div class="add"><span class="ic">${ICON[a.icon] || ICON.star}</span>
        <span><b>${esc(a.name)}</b><small>${esc(a.desc || "")}</small></span></div>`).join("");
  }
  ...
  function paintPackages() {
    $("#pkgGrid").innerHTML = S.data.packages.map((p) => `
      <article class="pkg rv${p.badge ? " hot" : ""}">
        ...
        <button class="btn ${p.badge ? "" : "ghost"}" data-pick="${esc(p.name)}">اطلبي العرض</button>
      </article>`).join("");
